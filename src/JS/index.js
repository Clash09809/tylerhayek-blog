...
function nNavigate() {  
   ...
   $("#Navigate").css({"width": nDiameter, "height": nDiameter, "margin-top": -nRadius, "margin-left": -nRadius });
...
$(window).on("load", function() {  
   ...
   // loadfade 
   setTimeout(function() { $(".loadfade").css({"opacity":"1"}); }, 1000);   
   ...
   $(".navigate").click(function(e) {
      ...
      $("#NavigateOverlay").add("#LoaderContainer").css({"display":"block"}).animate({ opacity: 1 }, 500);
   ...
   // resize 
   $(window).on("resize", function() { nNavigate(); }); 
...
$(document).ready(function() { 
   ...
   $(".primary-control").click(function () {  
      ...
      if ( $(window).width() > 900 ) { 
	     ...
	     if (done) {
			...
			var thisDisplay = $("#" + $(this).attr("id") + "Display"),
                notThis = $(".primary-control-display").not(thisDisplay);	 
		     
		    if (notThis.css("opacity") > 0) { clearTimeout(timeout); }
   ...
   // animate display *********    
   function animateDisplay(elem,val) { elem.css({"opacity": val }); }
...
$(document).ready(function() { 
	...
	// date *********
	var date = new Date("April 10, 2019").getTime();     
	...
	var interval = setInterval(function() {
		...
		function display(a,b) {  
			...
			var sec = $("#CountdownSection " + a + " .num");
...
$(document).ready(function() {
   ...
   // initialization *********
   var elem = document.getElementById("TypewriterText"),
...
$(document).ready(function() {                 
   ...
   $("#WelcomeForm").click(function(e) {    
	  ...
	  // init 
	  var header = $("#WelcomeForm #Header"),
          formButton = $("#WelcomeForm button"),	
          success = $("#WelcomeForm #Success"),		    
	      nameVal = $("#FN").val().trim(),
	      emailVal = $("#EA").val().trim(),
	      stateVal = $("#ST").val().trim(),
	  ...
	  if ( !(nSubmit) && !(header.hasClass("done")) ) {       
	     ...
	     $("#WelcomeForm").delay(500).animate({ backgroundColor: "rgba(0,0,0,0.3)" }, 500);
		 displayEl(formButton, true);   
	  } 
	   
	  // input   
	  if ( !(nSubmit) && ((i === 0) || (i === 1 && testLength.test(nameVal)) || (i === 2 && testEmail.test(emailVal)) || (i === 3 && testLength.test(stateVal))) ) {
          
		 if (!(i === 0)) { displayEl($("#WelcomeForm .formoption" + i), false);  } 
	  ...
	  if ( i === 4 ) {
		 ...
		 setTimeout(function() {
			...
			setTimeout(function() { nSubmit = false; }, 1000);  
   ...
   function displayEl(el,display) {    
      ...
      if (display) {
	     el.css({"display":"block"}).delay(500).animate({ opacity: 1 }, 500); 
	  }   
	  else {
         el.animate({ opacity: 0 }, 500, function() { $(this).css({"display":"none"}); }); 
	  }
...
$(document).ready(function() {              
   ...
   $("#CarouselControls img").click(function() { 
	  ...
	  if (done) {	
	     ...
	     // right
	     if (this.id === "Right") { (i < 18) ? i++ : i = 1; }
	  
	     // left
	     if (this.id === "Left") { (i > 1) ? i-- : i = 18; }  
...
$(document).ready(function() {  
	...
	$("#InfoButton").click(function() {
		if (done) {
		   ...
		   $("#InfoSection .col.two").animate({ right: "0px", opacity: 1 }, 2500, "easeOutQuart"); 
		 
		   setTimeout(function() { $("#InfoSection .fade-in").css({"opacity":"1"}); }, 1500);    
	...
	$("#InfoSection #Close").click(function() {
	   if (done) {
		  ...
		  $("#InfoSection").animate({ opacity: 0 }, 1000, function() { $(".reset-attr").removeAttr("style"); }); 
